...
export class DatepickerComponent implements OnInit, OnDestroy, ControlValueAccessor {
  // CHANGE
  @Input() isJalali: boolean | undefined = true;
  ...
  onDateSelect(event: Event, dateMeta: any) {
    ...
    if (this.isMultipleSelection() && this.isSelected(dateMeta) && Array.isArray(this.value)) {
  ...
  shouldSelectDate(dateMeta: any) {
    if (this.isMultipleSelection() && Array.isArray(this.value)) return this.maxDateCount != null ? this.maxDateCount > (this.value ? this.value.length : 0) : true;
    else return true;
  }
  ...
  selectDate(dateMeta: any) {
    let date;
    if (this.isJalali) {
      date = this.momentService.convertToGregorian(`${dateMeta.year}/${dateMeta.month + 1}/${dateMeta.day}`, 'YYYY/MM/DD')
    } else {
      date = this.formatDateMetaToDate(dateMeta);
    }
  ...
  isDateBetween(start: Date | Moment, end: Date | Moment, dateMeta: any) {
    ...
    if (start && end) {
      let date: Date | Moment = this.formatDateMetaToDate([dateMeta.year, dateMeta.month, dateMeta.day]);
  ...
  constrainTime(hour: number, minute: number, second: number, pm: boolean) {
    ...
    if (isMultiValue && Array.isArray(this.value)) {
      if (!this.value) {
      ...
      if (isMultiple && Array.isArray(this.value)) {
...
//     if (this.currentHour === undefined) {
//       // CHANGES
//       this.initTime(this.value || this.isJalali ? this.getMoment() : new Date());
//     }
//     this.updateInputfield();
//   }
//
...
//     if (this.initialized) {
//       // CHANGES
//       const date = this.isJalali ? (defaultDate ?? this.getMoment(defaultDate) ?? this.getMoment()) : (defaultDate || new Date());
//       this.currentMonth = this.isJalali ? (date as Moment).jMonth() : (date as Date).getMonth();
//       this.currentYear = this.isJalali ? (date as Moment).jYear() : (date as Date).getFullYear();
//       this.initTime(date);
//       this.createMonths(this.currentMonth, this.currentYear);
//     }
//   }
...
//   @Output() onBlur: EventEmitter<Event> = new EventEmitter<Event>();
//   @Output() onClose: EventEmitter<AnimationEvent> = new EventEmitter<AnimationEvent>();
//   @Output() onSelect: EventEmitter<Date | Moment> = new EventEmitter<Date | Moment>();
//   @Output() onClear: EventEmitter<any> = new EventEmitter<any>();
//   @Output() onInput: EventEmitter<any> = new EventEmitter<any>();
//   @Output() onTodayClick: EventEmitter<Date | Moment> = new EventEmitter<Date | Moment>();
//   @Output() onClearClick: EventEmitter<any> = new EventEmitter<any>();
...
//     this.attributeSelector = $UniqueComponentId();
//     this.panelId = this.attributeSelector + '_panel';
//     const date = this.defaultDate || (this.isJalali ? this.getMoment() : new Date());
//     this.createResponsiveStyle();
//     if (this.isJalali) {
//       this.currentMonth = (date as Moment).jMonth();
//       this.currentYear = (date as Moment).jYear();
//     } else {
//       this.currentMonth = (date as Date).getMonth();
//       this.currentYear = (date as Date).getFullYear();
//     }
//     this.yearOptions = [];
//     this.currentView = this.view;
//
...
//   getWeekNumber(date: Date | Moment) {
//     // CHANGES
//     let checkDate: Date | Moment;
//     if (this.isJalali) {
//       date = date as Moment;
//       checkDate = checkDate as Moment;
//       checkDate = this.getMoment(date.unix());
//       if (this.startWeekFromFirstDayOfYear) {
//         let firstDayOfWeek: number = +this.getFirstDateOfWeek();
//         checkDate.jDate(checkDate.jDate() + 6 + firstDayOfWeek - checkDate.jDay());
//       } else {
...
//       checkDate.jMonth(0);
//       checkDate.jDate(1);
//       return Math.floor(Math.round((time - checkDate.jMonth()) / 86400000) / 7) + 1;
//     } else {
//       date = date as Date;
//       checkDate = checkDate as Date;
...
//     let dayNo = 1;
//     // CHANGES
//     let today = this.isJalali ? this.getMoment() : new Date();
//     let weekNumbers = [];
//     let monthRows = Math.ceil((daysLength + firstDay) / 7);
//
//     for (let i = 0; i < monthRows; i++) {
...
//
//       if (this.showWeek) {
//         weekNumbers.push(this.getWeekNumber(this.isJalali ? this.getMoment([week[0].year, week[0].month, week[0].day]) : new Date(week[0].year, week[0].month, week[0].day)));
//       }
//
//       dates.push(week);
//     }
...
//
//   initTime(date: Date | Moment) {
//     this.pm = (this.isJalali ? (date as Date).getHours() : (date as Moment).hours()) > 11;
//
//     if (this.showTime) {
//       if (this.isJalali) {
//         this.currentMinute = (date as Moment).minutes();
//         this.currentSecond = (date as Moment).seconds();
//         this.setCurrentHourPM((date as Moment).hours());
//       } else {
//         this.currentMinute = (date as Date).getMinutes();
//         this.currentSecond = (date as Date).getSeconds();
//         this.setCurrentHourPM((date as Date).getHours());
//       }
//     } else if (this.timeOnly) {
//       this.currentMinute = 0;
//       this.currentHour = 0;
...
//   formatDateMetaToDate(dateMeta: any): Date | Moment {
//     if (this.isJalali) {
//       return this.getMoment([dateMeta.year, dateMeta.month, dateMeta.day])
//     } else {
...
//       if (this.hourFormat == '12') {
//         if (this.currentHour === 12) {
//           date[this.isJalali ? 'hours' : 'setHours'](this.pm ? 12 : 0)
//         } else {
//           date[this.isJalali ? 'hours' : 'setHours'](this.pm ? <number>this.currentHour + 12 : <number>this.currentHour)
//         }
//       } else {
//         date[this.isJalali ? 'hours' : 'setHours'](<number>this.currentHour);
//       }
//       date[this.isJalali ? 'minutes' : 'setMinutes'](<number>this.currentMinute);
//       date[this.isJalali ? 'seconds' : 'setSeconds'](<number>this.currentSecond);
//     }
//
//     if (this.minDate && this.minDate > date) {
//       date = this.minDate;
//       this.setCurrentHourPM(date[this.isJalali ? 'hours' : 'getHours']());
//       this.currentMinute = date[this.isJalali ? 'minutes' : 'getMinutes']();
//       this.currentSecond = date[this.isJalali ? 'seconds' : 'getSeconds']();
//     }
//
...
//         let endDate: Date | Moment = this.value[1];
//
//         if (!endDate && date[this.isJalali ? 'unix' : 'getTime']() >= startDate[this.isJalali ? 'unix' : 'getTime']()) {
//           endDate = date;
//         } else {
//           startDate = date;
//           endDate = null;
...
//       day.jMonth(month);
//       day.jYear(year);
//       dayIndex = day.jDay() + this.getSundayIndex();
//     } else {
//       day = new Date();
//       day.setDate(1);
//       day.setMonth(month);
//       day.setFullYear(year);
//       dayIndex = day.getDay() + this.getSundayIndex();
//     }
//     return dayIndex >= 7 ? dayIndex - 7 : dayIndex;
//   }
//
...
//     if (start && end) {
//       if (this.isJalali) {
//         let date = this.getMoment([dateMeta.year, dateMeta.month, dateMeta.day]) as Moment;
//         return (start as Moment).unix() <= date.unix() && (end as Moment).unix() >= date.unix();
//       } else {
//         let date = this.formatDateMetaToDate(dateMeta) as Date;
//         return (start as Date).getTime() <= date.getTime() && (end as Date).getTime() >= date.getTime();
//       }
//     }
//
//     return between;
...
//
//     if (this.minDate) {
//       if (this.minDate[this.isJalali ? 'jYear' : 'getFullYear']() > year) {
//         validMin = false;
//       } else if (this.minDate[this.isJalali ? 'jYear' : 'getFullYear']() === year && this.currentView != 'year') {
//         if (this.minDate[this.isJalali ? 'jMonth' : 'getMonth']() > month) {
//           validMin = false;
//         } else if (this.minDate[this.isJalali ? 'jMonth' : 'getMonth']() === month) {
//           if (this.minDate[this.isJalali ? 'jDate' : 'getDate']() > day) {
//             validMin = false;
//           }
//         }
//       }
...
//
//     if (this.maxDate) {
//       if (this.maxDate[this.isJalali ? 'jYear' : 'getFullYear']() < year) {
//         validMax = false;
//       } else if (this.maxDate[this.isJalali ? 'jYear' : 'getFullYear']() === year) {
//         if (this.maxDate[this.isJalali ? 'jMonth' : 'getMonth']() < month) {
//           validMax = false;
//         } else if (this.maxDate[this.isJalali ? 'jMonth' : 'getMonth']() === month) {
//           if (this.maxDate[this.isJalali ? 'jDate' : 'getDate']() < day) {
//             validMax = false;
//           }
//         }
//       }
...
//     if (this.disabledDates) {
//       for (let disabledDate of this.disabledDates) {
//         if (disabledDate[this.isJalali ? 'jYear' : 'getFullYear']() === year && disabledDate[this.isJalali ? 'jMonth' : 'getMonth']() === month && disabledDate[this.isJalali ? 'jDate' : 'getDate']() === day) {
//           return true;
//         }
//       }
//     }
...
//   isDayDisabled(day: number, month: number, year: number): boolean {
//     if (this.disabledDays) {
//       let weekday = this.isJalali ? this.getMoment([year, month, day]) : new Date(year, month, day);
//       let weekdayNumber = this.isJalali ? (weekday as Moment).jDay() : (weekday as Date).getDay();
//       return this.disabledDays.indexOf(weekdayNumber) !== -1;
//     }
//     return false;
//   }
...
//       case 33: {
//         cellContent.tabIndex = '-1';
//         const dateToFocus = new Date(currentDate[this.isJalali ? 'jYear' : 'getFullYear'](), currentDate[this.isJalali ? 'jMonth' : 'getMonth']() - 1, currentDate[this.isJalali ? 'jDate' : 'getDate']());
//         const focusKey = this.formatDateKey(dateToFocus);
//         this.navigateToMonth(true, groupIndex, `span[data-date='${focusKey}']:not(.p-disabled):not(.p-ink)`);
//         event.preventDefault();
//         break;
...
//       case 36:
//         cellContent.tabIndex = '-1';
//         const firstDayDate = new Date(currentDate[this.isJalali ? 'jYear' : 'getFullYear'](), currentDate[this.isJalali ? 'jMonth' : 'getMonth'](), 1);
//         const firstDayDateKey = this.formatDateKey(firstDayDate);
//         const firstDayCell = $DomHandler.findSingle(cellContent.offsetParent, `span[data-date='${firstDayDateKey}']:not(.p-disabled):not(.p-ink)`);
//         if (firstDayCell) {
//           firstDayCell.tabIndex = '0';
...
//       case 35:
//         cellContent.tabIndex = '-1';
//         const lastDayDate = new Date(currentDate[this.isJalali ? 'jYear' : 'getFullYear'](), currentDate[this.isJalali ? 'jMonth' : 'getMonth']() + 1, 0);
//         const lastDayDateKey = this.formatDateKey(lastDayDate);
//         const lastDayCell = $DomHandler.findSingle(cellContent.offsetParent, `span[data-date='${lastDayDateKey}']:not(.p-disabled):not(.p-ink)`);
//         if (lastDayDate) {
//           lastDayCell.tabIndex = '0';
...
//     if (isMultiValue) {
//       if (!this.value) {
//         this.value = [(this.isJalali ? this.getMoment() : new Date()), (this.isJalali ? this.getMoment() : new Date())];
//       }
//       if (isRange) {
//         value = this.value[1] || this.value[0];
//       }
...
//       value = this.value[this.value.length - 1];
//     }
//     value = value ? (this.isJalali ? this.getMoment(value.getTime()) : new Date(value.getTime())) : (this.isJalali ? this.getMoment() : new Date());
//
//     if (this.hourFormat == '12') {
...
//   }
//
//   parseValueFromString(text: string): Date | Moment | Date[] | Moment[] | null {
//     if (!text || text.trim().length === 0) {
//       return null;
//     }
//
...
//
//     if (this.timeOnly) {
//       date = this.isJalali ? this.getMoment() : new Date();
//       this.populateTime(date, parts[0], parts[1]);
//     } else {
//       const dateFormat = this.getDateFormat();
//       if (this.showTime) {
...
//
//   isValidDate(date: any) {
//     return (this.isJalali ? this.getMoment(date).isValid() : $ObjectUtils.isDate(date)) && $ObjectUtils.isNotEmpty(date);
//   }
//
//   updateUI() {
//     let propValue = this.value;
...
//     }
//
//     let val = this.defaultDate && this.isValidDate(this.defaultDate) && !this.value ? this.defaultDate : propValue && this.isValidDate(propValue) ? propValue : (this.isJalali ? this.getMoment() : new Date());
//
//     if (this.isJalali) {
...
//       extra,
//       iValue = 0,
//       shortYearCutoff = typeof this.shortYearCutoff !== 'string' ? this.shortYearCutoff : ((this.isJalali ? this.getMoment().jYear() : new Date().getFullYear()) % 100) + parseInt(this.shortYearCutoff, 10),
//       year = -1,
//       month = -1,
//       day = -1,
//       doy = -1,
...
//         selectable: true
//       };
//       this.createMonths(date.jMonth(), date.jYear());
//     } else {
//       date = new Date()
//       dateMeta = {
//         day: date.getDate(),
